void replay_cosmics_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  */
  ...
  // Define DEF-file
  analyzer->SetOdefFile("DEF-files/SHMS/COSMICS/pstackana_cosmics.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/SHMS/PRODUCTION/CUTS/pstackana_production_cuts.def");  // optional
  // File to record accounting information for cuts
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/SHMS/PRODUCTION/summary_cosmics_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/SHMS/PRODUCTION/pstackana_production.template",
  			Form("REPORT_OUTPUT/SHMS/PRODUCTION/replay_shms_cosmics_%d_%d.report", RunNumber, MaxEvent));  // optional
